    ...
    <q-layout-header>
      ...
      <q-toolbar v-if="currentWatchers.length > 0" color="blue-grey-8">
        ...
        <q-toolbar-title shrink v-for="(w,idx) in currentWatchers" :key="w.id" v-if="w.events.latest.candle">
          ...
          <img class="crypto-icon-16" :src="'statics/crypto_icons/color/' + w.config.watch.currency.toLowerCase() + '.svg'"
               :alt="w.config.watch.currency" :title="w.config.watch.currency">

          <span slot="subtitle">
              <img class="crypto-icon-16 q-icon q-mr-xs" :src="'statics/crypto_icons/color/' + w.config.watch.asset.toLowerCase() + '.svg'"
